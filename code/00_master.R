...
# 00_master.R
#
# Master orchestration script for the Distributional CPI / LA-AIDS thesis
# pipeline.
#
# PIPELINE OVERVIEW
# -----------------
...
# HOW TO USE
# ----------
#  1. Fill in every path in Section 1 below (only that section ever needs to
...
#     Or run it section-by-section interactively.
#
# NOTE ON SEPARATE PIPELINES
# ---------------------------
# The CPI and DCPI calculations are run as SEPARATE processes:
#   0_CPI/0a.master.R   — run standalone to replicate the official BLS CPI
#   8_DCPI/0a.master.R  — run standalone AFTER step 7 to produce Distributional
#                         CPIs using the fused budget shares from post_hurdle_dir
# This master does NOT trigger either of those pipelines.
#
# YEAR RANGES USED BY EACH STEP
# --------------------------------
#  Steps 1a–1c, 2a–2b   1999–2021  (pre-hurdle reshaping; fmli + fmld)
#  Step  3               1999–2021  (preliminary budget shares; fmli + fmld)
#  Steps 4–7             1999–2021  (hurdle model, fusion, final shares)
#  Step  9               2010–2019  (LA/AIDS demand system)
################################################################################
...
# ==============================================================================
# Edit ONLY this section when moving the project to a new machine or drive.
# All other sections read from these variables.
#
# NOTE: paths will be injected into each individual script during the next
# refactoring pass.  For now, keep this section in sync manually and treat it
# as the ground-truth directory map for the whole project.
# ==============================================================================
...
# ==============================================================================
# Create all output directories upfront so individual scripts never fail on a
# missing folder.
...
# Note:    Combines fmli (Interview) and fmld (Diary).  Years: 2008–2021.
#          This is a PRELIMINARY version used only as input to the hurdle model
#          (step 4).  The final, production budget shares come from step 7.
if (RUN_STEP_3) {
...
# combining Interview own-records with Diary donor records, with diary costs
# scaled to a 90-day reference window.
#
# Years covered: 1999–2021
# ==============================================================================
...
#          (one row per household × UCC × quarter; source_flag = interview_own
#           or diary_donor; diary costs scaled to 90-day equivalent)
if (RUN_STEP_5) {
...
# Note:    Functionally equivalent to step 2b but applied to fused spending and
#          extended back to 1999.
if (RUN_STEP_6) {
  message("\n=== STEP 6: Post-fusion wide category reshape ===")
...
# aggregation (Region × Month × Income quintile).  Uses Stone's Price Index
# for real expenditure deflation and includes demographic controls.
#
# Years covered: 2010–2019 (regional price data availability)
# Inputs:
#   post_hurdle_dir/{YYYY}/3_annualized_budget_shares_{YYYY}.csv   (step 7)
