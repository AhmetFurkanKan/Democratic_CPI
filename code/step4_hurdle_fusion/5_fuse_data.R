...
#           to build a fused vertical spending file on a QUARTERLY basis.
#
# Timeframe alignment:
#   Interview : already quarterly (3-month reference window) — no adjustment
#   Diary     : covers num_days (constant per HH, varies across HHs)
#               → scaled to quarterly: cost * (90 / num_days)
#
...
QUARTERLY_DAYS  <- 90   # interview reference window in days
...
if (!exists("years_to_model")) {
  years_to_model <- 1999:2021  # Set your desired year range here
}
...
for (yr in years_to_model) {
  ...
  # Define dynamic paths for the current year
  ...
  # Safely check if files exist before proceeding
  ...
  ################################################################################
  # Step 4: Quarterly-align diary spending
  ################################################################################
  cat("\nAligning diary spending to quarterly window...\n")
  ...
  if (nrow(num_days_check) > 0) {
    ...
    cat("Using row-level num_days for scaling (cost * 90 / num_days per row).\n")
  } else {
    cat("Validation passed: num_days is constant within every Diary HH.\n")
  }
  
  # Check for zero or missing num_days before dividing
  n_zero_days    <- sum(diary_spending$num_days == 0,  na.rm = TRUE)
  n_missing_days <- sum(is.na(diary_spending$num_days))
  
  if (n_zero_days > 0)
    warning(sprintf("%d diary rows have num_days = 0 — these will produce Inf costs. Investigate.", n_zero_days))
  if (n_missing_days > 0)
  ...
  # --- Block B: Diary donor quarterly rows ---
  # Extract interview HH demographics (one row per inter_newid)
  ...
  diary_rows <- match_pairs %>%
    ...
    mutate(
      ...
      diary_donor_newid = diary_newid,
      newid             = inter_newid,
      # OVERWRITE fmld with fmli to treat as part of Interview HH
      #i.data            = INTERVIEW_LABEL 
    ) %>%  
    select(-inter_newid, -diary_newid)
  ...
  cat("                       interview rows: original quarterly values\n")
  cat("                       diary rows:     cost * (90 / num_days)\n")
  ...
  # ------------------------------------------------------------------
  # Step 8: Explicit Memory Cleanup
  # ------------------------------------------------------------------
  # Remove all the large dataframes created during this iteration
  ...
  # Force R to immediately release the RAM back to your computer
  gc() 
} 
