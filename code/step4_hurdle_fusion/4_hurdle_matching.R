# Step 4 — Two-Part Hurdle Model + Data Fusion
# Structure:
#   Stage 1 — Participation models (LASSO logistic)
#   Stage 2 — Intensity models    (LASSO linear on log-share)
#   Stage 3 — Matching: one row per Interview HH, Diary HH is the donor. Donor selected by uniform sampling among k nearest neighbors.
#
# Output CSV columns:
...
DIARY_LABEL     <- "fmld"   # Diary records
INTERVIEW_LABEL <- "fmli"   # Interview records
K_CANDIDATES    <- 10       # nearest Diary neighbours to consider per Interview 
SEED            <- 42       
...
prepare_data <- function(data) {
  ...
  # Apply 0.1% threshold for broad 29 categories 
...
# Stage 2: Intensity Models (LASSO on log-share)
################################################################################
...
predict_shares <- function(data, part_res, int_res, categories) {
  ...
  for (current_cat in categories) {
    if (current_cat %in% names(part_res$models) &&
      ...
      # Duan's Smearing Estimator to correct retransformation bias
      ls_col <- paste0("logshare_", current_cat)
      p_col  <- paste0("participate_", current_cat)
...
#
# Base table  : Interview households
# Donor pool  : Diary households     
#
...
perform_data_fusion <- function(data_with_preds, categories) {
  ...
  to_numeric_matrix <- function(df, cols) {
    ...
    for (col in names(mat)) {
      # If the column is categorical (character or factor)
      if (is.character(mat[[col]]) || is.factor(mat[[col]])) {
        mat[[col]] <- as.numeric(as.factor(as.character(mat[[col]])))
      } else {
        # If it is already numeric (like your pred_ variables), LEAVE IT ALONE
  ...
  for (i in seq_len(n_inter)) {
    ...
    # Uniform sampling among k candidates
    chosen_position       <- sample(k_actual, size = 1)
...
for (yr in years_to_model) {
  ...
  # 2. Run the pipeline for this year
